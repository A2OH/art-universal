...
void Thread::ThrowNewWrappedException(const char* exception_class_descriptor,
  ...
  // PATCH: During AOT compilation (dex2oat), skip EnsureInitialized for exception classes.
  // EnsureInitialized triggers VerifyClass, which can re-enter VerifyClass on a class
  // already being verified (e.g., Object), causing a single-thread deadlock.
  // In AOT mode, exception throwing is just for recording metadata; we don't need
  // fully initialized exception classes.
