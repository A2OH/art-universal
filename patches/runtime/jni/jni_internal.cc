...
#define SET_STATIC_PRIMITIVE_FIELD(fn, value) \
  ...
  static jint RegisterNatives(JNIEnv* env,
    ...
    for (jint i = 0; i < method_count; ++i) {
      ...
      if (m == nullptr) {
        // Tolerant: skip missing methods instead of throwing (standalone build)
        fprintf(stderr, "[jni] Skipping missing method: %s%s\n", name, sig);
        if (soa.Self()->IsExceptionPending()) soa.Self()->ClearException();
        continue;
      } else if (!m->IsNative()) {
        fprintf(stderr, "[jni] Skipping non-native method: %s%s\n", name, sig);
        if (soa.Self()->IsExceptionPending()) soa.Self()->ClearException();
        continue;
      }
